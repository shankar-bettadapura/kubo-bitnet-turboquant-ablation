...
class BitLinear(nn.Linear):
    def forward(self, x):
        w = self.weight
        # quantize weights to ternary {-1, 0, +1} using straight-through estimator
        w_scale = w.abs().mean()
        w_ternary = (w / (w_scale + 1e-8)).clamp(-1, 1)
        w_ternary = (w_ternary.sign() * (w_ternary.abs() > 0.5).float()).detach() + w_ternary - w_ternary.detach()
        return F.linear(x, w_ternary * w_scale, self.bias)
